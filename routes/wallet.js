          ...
          .then(async function (cart) {
            ...
            const  getTotalAmount = () => {
              for (let value of newCart) {
                ...
                let totalContainer = Number(value.menu.containerAmount) * Number(value.quantity)
                 totalAmount = totalAmount + totalValue +  totalContainer;
              }
            }
...
router.put("/wallets/:id", function (req, res, next) {
  Wallet.findByIdAndUpdate({ _id: req.params.id }, req.body).then(function (
    wallets
  ) {
...
router.get("/transaction/:id", function (req, res, next) {
  Transaction.find({user: req.params.id})
    .then(function (newtransaction) {
      let newtransaction =   newtransaction.reverse();
